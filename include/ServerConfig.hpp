...
struct ServerConfig
{
    ...
    ServerConfig(uint16_t _serverPort, const std::string& _host, const std::string& _serverName,
                 const std::string& _serverRoot, const std::string& _indexPage,
                 const std::string& _clientMaxBody, const std::string& _errorPage,
                 const std::string& _socketIp)
        : serverPort(_serverPort), host(_host), serverName(_serverName), serverRoot(_serverRoot),
          indexPage(_indexPage), clientMaxBody(_clientMaxBody), errorPage(_errorPage), socketIp(_socketIp) {}
};
